class DireccionsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @direccion.update(direccion_params)
        format.html { redirect_to @direccion, notice: 'Registro de Direccion modificado exitosamente.' }
  ...
  def destroy
    @direccion.destroy
    respond_to do |format|
      format.html { redirect_to direccions_url, notice: 'Registro de Direccion destruido.' }
  ...
  private
    ...
    def direccion_params
      params.require(:direccion).permit(:calle, :numero, :comuna, :ciudad, :region, :pais, :trabajador_id)
    end
end
